...
function useVRM(gltf,label){vrm=gltf.userData.vrm;if(!vrm)throw new Error('GLTF loaded, but gltf.userData.vrm is empty.');VRMUtils.rotateVRM0(vrm);scene.add(vrm.scene);fit(vrm.scene);title.textContent='✅ Character loaded';detail.textContent=`${label} • VRM ${vrm.meta?.metaVersion||'detected'}`;tryAutoWearable();}
...
vrmLoader.load('./moe-beginner.vrm?v=4b1',gltf=>{try{useVRM(gltf,'Loaded from GitHub Pages');}catch(e){showError(e);}},p=>{if(p.total)detail.textContent=`Downloading character… ${Math.round(p.loaded/p.total*100)}%`;else detail.textContent=`Downloading character… ${Math.round(p.loaded/1024/1024)} MB`;},e=>showError(e));
...
function tryAutoWearable(){loadWearableUrl('./wearables/english-hoodie.glb?v=4b1','english-hoodie.glb',()=>{state.textContent='No bundled hoodie yet. Choose a .glb wearable from your computer when you have one.';state.classList.add('warn');});}
